...
(async () => {
    ...
    async function scrapeCurrentPage() {
        // Find agent links
        // Based on analysis: links contain /agentprofile/ or /realestateagents/ and usually end with an ID
        ...
        links.forEach(link => {
            ...
            // Extract ID: usually last part or query param
            // Example: .../agentname_id
            try {
                const parts = href.split("/");
                const lastPart = parts[parts.length - 1].split("?")[0];
                if (lastPart && (href.includes("/agentprofile/") || href.includes("/realestateagents/"))) {
                     // Simple heuristic: if it looks like an ID (often has underscores or is unique)
                     // Verification: real IDs are often alphanumeric.
                     // Let's rely on the strategy from main.py: split("/")[-1]
    ...
    try {
        ...
        while (running) {
            ...
            for (const [id, url] of agents) {
                ...
                if (details) {
                    // Only save if phone exists (per user preference? or save all?)
                    // The python code had a commented out check # if not phone: return
                    // We will save all for now, or match Python behavior if it was active.
                    // Python: save_row is called always.
